class PaymentsController < ApplicationController
 ...
 after_filter :check_privilege_and_renewal_date, :only => [:update]
  ...
  def list_by_member_class
    ...
    @pays = Payment.find :all,  :include => :privilege ,:conditions => "paymenttype_id in ('1','4','5') and  date_lodged > '#{this_yr_start}'", :order => 'privileges.name, date_lodged' 
  end
...
WHERE    (date_lodged >= '#{date_start}'
  ...
  def create
    ...
    respond_to do |format|
      if @payment.save
        if @payment.paymenttype_id == 1 or @payment.paymenttype_id == 4
          @member.renew_date = @payment.date_lodged unless @payment.date_lodged < @member.renew_date
          @member.save!
        end
        flash[:notice] = 'Payment Successfully Created.'
        if @member.privilege_id != @payment.privilege_id
           @member.privilege_id = @payment.privilege_id
           if @member.save!
             flash[:notice] = 'Payment Successfully Created, Membership Class updated.'
           end
        end
        format.html { redirect_to :controller =>'people', :action => 'edit',:id => Person.main_person(params[:payment][:member_id]).id   }
      else
...
private 
  ...
  def check_privilege_and_renewal_date
    if (@payment.date_lodged.year == Time.now.year)  &&  renewal_payment
      @person.member.privilege_id = @payment.privilege_id if check_privilege
