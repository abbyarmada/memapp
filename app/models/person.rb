class Person < ActiveRecord::Base
  ...
  scope :current, ->  { joins(:member).where( 'members.renew_date >= ? or (members.renew_date BETWEEN ? and ? and date(?) <= date(?) ) ' , Time.now.beginning_of_year,1.year.ago.beginning_of_year,1.year.ago.end_of_year,Time.now, Time.now.year.to_s + "-05-01" )  }
  ...
  def self.search(params)
    ...
    #people = people.joins({:members => :privilege},:peoplebarcard,:loyaltycard).order(:last_name, :first_name).paginate(:per_page => 30, :page => params[:page])
