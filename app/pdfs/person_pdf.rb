class PersonPdf < Prawn::Document
  ...
  def initialize(person)
    ...
    number_pages("Page <page> of <total>, MYC Renewal #{Time.now.utc.year}, #{@person.first_name + ' ' + @person.last_name}", options = {
                   at: [bounds.right / 3, 0],
                   width: 150,
                   align: :center,
                   page_filter: (1..7),
  ...
  def address_txt
    ...
    text Time.now.utc.to_date.to_s(:long).to_s, size: 8, style: :normal, horizontal_padding: 2
  end
  ...
  def cart
    ...
    cart_totals = []
    cart << ['Description', ' ', ' ', ' ', ' ']
    cart << ["#{@person.member.privilege.name} Membership Renewal", ' ', ' ', ' = ', number_to_euro(@person.member.privilege.subscriptions.thisyear(@person.member.privilege.id)).to_s]
    ...
    if num_pentags > 0
      @person.member.boats.windsurfer.other.each do |penboats|
        cart << ['Pen Fees ', penboats.boat_type.to_s, penboats.boat_class.to_s, ' = ', number_to_euro(40).to_s]
        cart_totals << [40]
      end
      @person.member.boats.windsurfer.kona.each do |penboats|
        cart << ['Pen Fees ', penboats.boat_type.to_s, penboats.boat_class.to_s, ' = ', number_to_euro(60).to_s]
        cart_totals << [60]
      end
      ...
      @person.member.boats.dinghy.laser.each do |penboats|
        cart << ['Pen Fees ', penboats.boat_type.to_s, penboats.boat_class.to_s, ' = ', number_to_euro(50).to_s]
        cart_totals << [50]
      end
      @person.member.boats.dinghy.oppi.each do |penboats|
        cart << ['Pen Fees ', penboats.boat_type.to_s, penboats.boat_class.to_s, ' = ', number_to_euro(30).to_s]
        cart_totals << [30]
      end
      @person.member.boats.dinghy.canoe.each do |penboats|
        cart << ['Pen Fees ',
                 penboats.boat_type.to_s,
                 penboats.boat_class.to_s,
                 ' = ',
                 number_to_euro(00).to_s]
        cart_totals << [00]
      end
    end
    cart_sum = eval cart_totals.join('+')
    cart << ['Total', ' ', ' ', '  ', number_to_euro(cart_sum).to_s]
  end
  ...
  def memberloyalty
    @person.member.people.map do |person|
      [
        person.status == 'g' ? '' : person.first_name,
        loyaltypoints(person)
      ]
    end
  end
  ...
  def display_loyalty_points?
    return false if @person.loyaltycard.Current_Points.blank?
    @person.loyaltycard.Current_Points.to_f > 0 && @person.loyaltycard.RedeemedTD.to_i < 11
  end

  def loyaltypoints(person)
    return '' if person.status == 'g'
    loyalty_points = person.loyaltycard.Current_Points.present? ? person.loyaltycard.Current_Points.to_f / 100 : 0
    number_to_euro(loyalty_points)
  end

  def member_postal_address(top)
