...
describe Person, type: :model do
  ...
  #   # 'erroring person status m'
  #   expect(create(:person, status: 'p', member_id: 1)).to be(false)
  # end
  ...
  it 'Determines a persons partner' do
    person = create(:person)
    create(:person, first_name: 'Jane', status: 'p', member_id: person.member_id)
  ...
  it 'Age is calculated correctly' do
    person = create(:person, dob: (Time.zone.today - 10.years))
    expect(person.age).to eq(10)
  end
  it 'Determins an adult correctly' do
    person = create(:person, dob: (Time.zone.today - 18.years))
    expect(person.adult?).to eq(true)
  end
  it 'Determins an adult correctly < 18 ' do
    person = create(:person, dob: (Time.zone.today - 11.years))
  ...
  it "Finds the member last name beginning 'Sm'" do
    ...
    create(:person, member: member, last_name: 'Smith')
  ...
  it 'Counts the number of members - Grouped' do
    ...
    member = create(:member, privilege: privilege)
    create(:person, member: member)
    create(:person, status: 'ch', member: member)
